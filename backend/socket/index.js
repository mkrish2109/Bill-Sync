...
const initializeSocket = (server) => {
  ...
  io.on("connection", (socket) => {
    // Room management
    socket.on("join_room", (userId, callback) => handleJoinRoom(socket, userId, callback));
    socket.on("get_rooms", (callback) => handleGetRooms(socket, callback));

    // Connection events
    socket.on("disconnect", (reason) => handleDisconnect(socket, reason));
    socket.on("error", (error) => handleError(socket, error));
    socket.on("reconnect_attempt", (attemptNumber) => handleReconnectAttempt(socket, attemptNumber));
    socket.on("reconnect", (attemptNumber) => handleReconnect(socket, attemptNumber));
    socket.on("reconnect_error", (error) => handleReconnectError(socket, error));
