...
exports.sendRequest = async (req, res) => {
  try {
    ...
    if (!io) {
      ...
      return sendErrorResponse(
        res,
        "Server error: Socket.io not initialized",
        500
      );
    }
    ...
    if (!workerRoom) {
      console.warn(
        `Worker's room ${workerRoomId} not found. Worker might be offline.`
      );
    }
...
exports.getUserRequests = async (req, res) => {
  try {
    ...
    sendDataResponse(res, {
      ...
      connections:
        userType === "Worker"
          ? await Worker.findById(userModel._id).populate(
              "connectedBuyers",
              "name contact"
            )
          : await Buyer.findById(userModel._id).populate(
              "connectedWorkers",
              "name contact"
            ),
    });
